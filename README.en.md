...
When an ordinary user produces new reusable enhancement experience, do not directly modify or commit $HOME/.os-enhance-skill; create a local patch under $HOME/.os-enhance-skill-patches/ so the skill repository can continue to use git pull --ff-only. Developers with repository write access should maintain the repository according to their current development environment's global prompts or project rules.
```
...
Ordinary users should keep new local experience under `$HOME/.os-enhance-skill-patches/` instead of committing directly to `$HOME/.os-enhance-skill`. This keeps the skill repository able to pull upstream updates with `git pull --ff-only`; repository developers can later integrate those patches into `references/` or `knowledge/` and maintain the repository according to their own environment rules.
