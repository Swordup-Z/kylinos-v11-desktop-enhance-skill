...
When an ordinary user produces new reusable enhancement experience, do not directly modify or commit $HOME/.os-enhance-skill; create a local patch under $HOME/.os-enhance-skill-patches/ so the skill repository can continue to use git pull --ff-only. Maintain the repository directly only after explicitly entering maintainer mode, confirming osSkill.maintainer=true for $HOME/.os-enhance-skill, and passing a dry-run push.
```
...
Ordinary users should keep new local experience under `$HOME/.os-enhance-skill-patches/` instead of committing directly to `$HOME/.os-enhance-skill`. This keeps the skill repository able to pull upstream updates with `git pull --ff-only`; repository maintainers can run this optional check before integrating patches into `references/` or `knowledge/` and maintaining the repository according to their own environment rules.
