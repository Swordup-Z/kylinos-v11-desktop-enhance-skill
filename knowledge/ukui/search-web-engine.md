...
如果问题是全局搜索异常或软件商店结果异常，而不是新增搜索引擎，切换到 `$HOME/.os-fix-skill/SKILL.md`，再按 `references/ukui.md` 路由。如果问题是通用源码重编译策略，而不是本搜索引擎场景，改读 `knowledge/source-rebuild/README.md`。
