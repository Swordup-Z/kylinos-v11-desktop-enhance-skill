...
本文档用于记录所有“通过本地修改系统组件源码并替换系统产物实现功能增强或本地客制化”的本机工作区索引规则。DATA 工作区记录当前机器的源码、构建、回滚和现场 patch；可跨机器复用的功能 patch 还必须同步放入本 skill 的对应 knowledge 场景目录，例如 `knowledge/feature-enhancement/ukui/patches/<feature-id>/`。如果目标是修复已有系统故障，切换到 `$HOME/.os-fix-skill`。

## 适用场景

- 用户要求保留本地修改源码，方便后续继续迭代。
- 增强需要替换系统共享库、控制中心插件、系统服务或 schema。
...
```text
knowledge/feature-enhancement/<scenario>/patches/<feature-id>/
├── PATCHSET.md
└── <patch-files>
```
...
`CUSTOMIZATION.md` 中增加“源码包与 patch 映射”表：
...
全局索引和项目索引中的状态必须按阶段更新，不能提前确认。阶段含义：
...
如果从本地客制化中抽象出通用增强经验，应写到对应 `knowledge/feature-enhancement/<scenario>/<topic>.md` 或组件场景章节，并使用 `$HOME`、`<component-or-fix>`、`<timestamp>` 等占位符。功能级源码 patch 应放入对应场景的 `patches/<feature-id>/`，并通过 `PATCHSET.md` 说明上游仓库、基线节点和合入策略。修复类经验和 patch 应沉淀到 `$HOME/.os-fix-skill`。
