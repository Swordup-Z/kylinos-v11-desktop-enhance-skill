...
此文档用于初始化 Codex、Claude Code、opencode 等 AI 编程工具的用户级全局提示词，使其在处理 KylinOS Desktop V11 系统修复或功能增强问题时，按需加载共享经验库 `$HOME/.os-fix-skill/SKILL.md` 或 `$HOME/.os-enhance-skill/SKILL.md`，而不是把全部经验一次性塞入上下文。
...
- 两个 skill 本体分别放在 `$HOME/.os-fix-skill/` 和 `$HOME/.os-enhance-skill/`，作为多个 AI 工具共用的本机经验库；不要只放到某个工具的内置 skills 目录。文档内部使用 `$HOME`、`<user>`、`<app-id>` 等占位符，不写死具体用户名。
...
- 使用渐进式披露：先读命中的 `SKILL.md`，再按 `references/system-repair/<scenario>.md` 或 `references/feature-enhancement/<scenario>.md` 读取具体场景；如果 reference 指向 `knowledge/` 章节，再按需读取对应章节。不要预加载所有 reference 或 knowledge。
- 如果工具原生支持 `SKILL.md` 发现机制，可以同时把两个 skill 安装/同步到工具支持的全局 skills 目录；如果不支持，则依靠全局提示词显式要求读取共享路径下的 `SKILL.md`。
- 系统级修复或增强规则不依赖具体工具：修改 `/usr`、`/etc`、`/opt`、系统包、系统服务、设备节点、分区、KSaf 策略等之前，必须先运行 `mm-cli -s` 并确认维护模式。
...
- 如果当前任务不是上述桌面系统修复或增强问题，例如普通代码开发、文档编辑、Git 操作、业务需求分析或通用软件问题，不要读取这两个 skill，也不要预加载其中的 `references/` 或 `knowledge/`。
- 开始处理系统修复问题前，先读取 `$HOME/.os-fix-skill/SKILL.md`，再按其中“参考文档路由”选择性读取 `references/system-repair/<scenario>.md`。如果 reference 指向 `knowledge/` 下的具体章节，再按需读取对应 knowledge。如果没有命中专门 reference，系统问题修复至少读取 `$HOME/.os-fix-skill/references/system-repair/system.md`。
- 开始处理系统功能增强前，先读取 `$HOME/.os-enhance-skill/SKILL.md`，再按其中“参考文档路由”选择性读取 `references/feature-enhancement/<scenario>.md`。如果 reference 指向 `knowledge/` 下的具体章节，再按需读取对应 knowledge。如果没有命中专门 reference，系统功能增强至少读取 `$HOME/.os-enhance-skill/references/feature-enhancement/system.md`。
...
```text
请帮我安装这两个 KylinOS Desktop V11 系统经验库：

https://github.com/Swordup-Z/kylinos-v11-desktop-fix-skill

要求：
1. 将修复经验库克隆到 $HOME/.os-fix-skill，并将功能增强经验库克隆到 $HOME/.os-enhance-skill，作为 Codex、Claude Code、opencode 等工具共用的本机经验库。
2. 根据当前工具类型，更新用户级全局提示词文件：
   - Codex: $HOME/.codex/AGENTS.md
   - Claude Code: $HOME/.claude/CLAUDE.md
   - opencode: $HOME/.config/opencode/AGENTS.md
3. 在全局提示词中加入规则：修复类问题先读取 $HOME/.os-fix-skill/SKILL.md，再按 references/system-repair/<scenario>.md 路由；增强类任务先读取 $HOME/.os-enhance-skill/SKILL.md，再按 references/feature-enhancement/<scenario>.md 路由。如果当前任务不是桌面系统修复或增强问题，不要加载这两个 skill。
...
如果工具支持通过配置文件引用额外 instruction 文件，全局提示词文件可保持很短，只引用 `$HOME/.os-fix-skill/SKILL.md` 和 `$HOME/.os-enhance-skill/SKILL.md`。但不要依赖符号链接或非官方导入语法，除非该工具文档明确支持。

## 初始化验证

```bash
test -f "$HOME/.os-fix-skill/SKILL.md" && sed -n '1,80p' "$HOME/.os-fix-skill/SKILL.md"
test -f "$HOME/.os-enhance-skill/SKILL.md" && sed -n '1,80p' "$HOME/.os-enhance-skill/SKILL.md"
test -f "$HOME/.codex/AGENTS.md" && rg -n '\\.os-fix-skill|\\.os-enhance-skill|mm-cli -s|磐石架构' "$HOME/.codex/AGENTS.md"
test -f "$HOME/.claude/CLAUDE.md" && rg -n '\\.os-fix-skill|\\.os-enhance-skill|mm-cli -s|磐石架构' "$HOME/.claude/CLAUDE.md"
test -f "$HOME/.config/opencode/AGENTS.md" && rg -n '\\.os-fix-skill|\\.os-enhance-skill|mm-cli -s|磐石架构' "$HOME/.config/opencode/AGENTS.md"
test -f "$HOME/desktop-develop/AGENTS.md" && rg -n 'kylin-space-guard|computer-use|\\.os-fix-skill|\\.os-enhance-skill' "$HOME/desktop-develop/AGENTS.md"
```
...
预期回答应说明：修复类问题先读取 `$HOME/.os-fix-skill/SKILL.md` 并进入 `references/system-repair/`；增强类任务先读取 `$HOME/.os-enhance-skill/SKILL.md` 并进入 `references/feature-enhancement/`；如果 reference 指向 `knowledge/`，再读取对应知识章节。
