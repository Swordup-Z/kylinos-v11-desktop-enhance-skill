...
如果问题是全局搜索异常或软件商店结果异常，而不是新增搜索引擎，切换到 `$HOME/.os-fix-skill/SKILL.md`，再按 `references/system-repair/ukui.md` 路由。如果问题是通用源码重编译策略，而不是本搜索引擎场景，改读 `knowledge/feature-enhancement/source-rebuild/README.md`。
...
如果用户只要求隐藏 360、搜狗等旧搜索引擎，不需要删除后端旧 URL 分支，也不要删除系统包自带旧图标资源；仅从设置插件下拉框中移除对应 `insertItem`。这样能避免已有旧 `gsettings` 值、手工配置或未来回滚时产生额外破坏。
