...
本文档用于记录所有“通过本地修改系统组件源码并替换系统产物实现功能增强或本地客制化”的本机工作区索引规则。DATA 工作区记录当前机器的源码、构建、回滚和现场 patch；可跨机器复用的功能 patch 还必须同步放入本 skill 的对应 knowledge 场景目录，例如 `knowledge/ukui/patches/<feature-id>/`。如果目标是修复已有系统故障，切换到 `$HOME/.os-fix-skill`。
...
如果从本地客制化中抽象出通用增强经验，应写到对应 `knowledge/<scenario>/<topic>.md` 或组件场景章节，并使用 `$HOME`、`<component-or-fix>`、`<timestamp>` 等占位符。功能级源码 patch 应放入对应场景的 `patches/<feature-id>/`，并通过 `PATCHSET.md` 说明上游仓库、基线节点和合入策略。修复类经验和 patch 应沉淀到 `$HOME/.os-fix-skill`。
