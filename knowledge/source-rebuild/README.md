...
本目录记录 KylinOS Desktop V11 上“需要重新编译系统源码包或替换系统二进制才能新增能力、改变默认行为或保留本地客制化”的增强流程。这里不记录普通应用安装经验，也不记录一次性现场日志；如果任务变成安装失败或系统级故障修复，切换到 `$HOME/.os-fix-skill/SKILL.md`。
...
- UKUI 系统托盘隐藏区稳定性属于修复类源码修改，切换到 `$HOME/.os-fix-skill/SKILL.md`，再按 `$HOME/.os-fix-skill/references/ukui.md` 路由。

只读取与当前源码修改匹配的一项子分类。需要本地源码目录、现场 patch、回滚和索引策略时才读取 `local-customization-index.md`；需要跨机器复用 patch 或迁移到新系统版本时才读取 `patch-library.md`。没有命中时停止，不要继续遍历源码或 patch 目录。
...
8. 如果决定安装，优先构建 `.deb` 并通过包管理器安装；只有在明确记录备份、回滚和验证步骤后，才考虑手动替换单个系统文件。
   手动替换系统包管理的文件只能保证当前文件内容生效，不能防止后续 `apt install --reinstall`、包升级或系统修复把文件覆盖回官方版本。若用户明确要求本地定制跨包重装/升级继续生效，应在维护模式下使用 `dpkg-divert --local --rename --add --divert <target>.distrib <target>` 建立本地转移：让包管理器后续写入 `<target>.distrib`，本地定制文件保留在 `<target>`。建立 diversion 前先把官方文件放回 `<target>`，让 `dpkg-divert` 移到 `.distrib`；随后再安装定制文件到原路径。记录 diversion 列表、官方 `.distrib` 哈希、定制文件哈希和撤销步骤。
...
`CUSTOMIZATION.md` 用来保存后续继续修改所需的信息，不写进一次性聊天记录。记录内容：
...
- 如果安装目标是系统包管理的单文件替换，记录是否使用 `dpkg-divert` 防止包重装/升级覆盖；记录原路径、`.distrib` 路径、定制文件来源、官方文件来源和撤销方法。
