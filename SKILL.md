...
- 完成新的可复用增强后，默认生成 `$HOME/.os-enhance-skill-patches/` 下的本地 patch，保持 skill 仓库干净以便拉取上游；只有显式进入开发者维护模式、确认 `osSkill.maintainer=true` 且 dry-run push 通过时，才直接更新对应 `references/` 路由和 `knowledge/` 章节，并按当前开发环境规则提交和维护仓库。
...
7. 沉淀经验：默认生成本地 patch；开发者维护模式下新增或更新对应 `knowledge/<scenario>/` 章节。
